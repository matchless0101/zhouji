...
| [云同步部署与真机联调](docs/08-云同步部署与真机联调.md) | 开发、运维 | 生产迁移、双设备验收与开放开关前的勾选清单 |
...
4. 账户同步增量阅读[云同步 PRD](docs/05-云同步产品需求增量.md)：R1–R5 已确认；同步客户端与协议已实现，默认关闭，开放前须完成联调清单
...
| 本地存储 | SwiftData |
| V1–V3 最低系统 | iOS 17 |
| 账户与后端 | V3 后新增迭代：Python + FastAPI + MySQL；Apple / 微信登录已完成；云同步可靠性修复已部署、客户端默认关闭；备份与定时任务已验证，待真机验收及外部运维配置 |
| 旧系统兼容 | V3 完成后单独评估 |
...
| 数据 | SwiftData 游客/账户独立本机库、任务/目标软删除、计时名称快照 |
| 主题与辅助功能 | 浅色/深色模式、动态字体、VoiceOver 标签、减少动态效果 |
| 账户 | 游客可用；Apple / 微信登录（均有真机成功记录）、钥匙串会话、昵称头像；云同步客户端已实现但默认关闭 |
| 数据保护 | 版本化 JSON 导出/恢复；服务端备份脚本与健康探针；见云同步阶段 A |
| 数据保护 | JSON v2 与 v1 兼容、恢复保护副本与失败回退、同归属恢复；迁移已有样本测试，生产备份演练仍待验收 |
...
本机切换、过期及注销保留规则见[账户隔离说明](docs/07-本机账户数据隔离.md)。云同步仍处于测试阶段，真机验收和对用户开放是两个独立步骤。
